    ...
    oTable = $('#otable').DataTable(
        ...
        $.extend(true, {}, CONSTANT.DATA_TABLES.DEFAULT_OPTION, {
            ...
            "ajax": function (data, callback, settings) {
                //封装请求参数
                var param = recordManage.getQueryCondition(data);
                $.ajax({
                    type: "GET",
                    url: "/record/page",//TODO
                    cache: false, //禁用缓存
                    data: param, //传入已封装的参数
                    dataType: "json",
...
var recordManage = {
    ...
    getQueryCondition: function (data) {
        ...
        //组装查询参数
        param.fuzzySearch = recordManage.fuzzySearch;
        if (recordManage.fuzzySearch) {//模糊查询
            param.fuzzy = $("#fuzzy-search").val();
        } else {//非模糊查询
            param.telephone = $("#username").val();
            param.isbn = $("#isbn").val();
        }
        //组装分页参数
...
$('#otable').on('click', 'a.op_freezeOrActive', function (e) {
    ...
    var nRow = $(this).parents('tr')[0];
    var id = oTable.row(nRow).id();
    var url = "/record/" + id + "/freezeOrActive";
